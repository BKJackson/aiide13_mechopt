...
\todo{Don't just enumerate, pull out the principles, like in the preceding paragraph. This paragraph should be shorter.}
...
For performance optimization we found the mean squared error of predictions against actual performance levels of IBO to be significantly less ($p<???$ \todo{fill}) than that of random sampling when comparing on a wave-by-wave basis. 
For control optimization we found the IBO model accuracy for predicting player choices was significantly higher ($p<???$ \todo{fill}) on a wave-by-wave basis. Thus, IBO uses human playtesting more effectively to automatically model a design space.
